...
import { ArrowLeft, Check, BookOpen, ChevronDown, Shield, Sparkles, Quote, X } from 'lucide-react'
import Link from 'next/link'
import { companions, type CompanionProfile } from '@/lib/data/companions'
import { BlurFade } from '@/components/effects/BlurFade'

const STORAGE_KEY = 'kalam-companions-read'

function getReadCompanions(): string[] {
...
function setReadCompanions(slugs: string[]) {
  try {
    localStorage.setItem(STORAGE_KEY, JSON.stringify(slugs))
  } catch { /* noop */ }
}

export default function CompanheirosClient() {
  const [readSlugs, setReadSlugs] = useState<string[]>([])
  const [openCompanion, setOpenCompanion] = useState<string | null>(null)
  const [readingCompanion, setReadingCompanion] = useState<CompanionProfile | null>(null)

  useEffect(() => {
    setReadSlugs(getReadCompanions())
  }, [])
  ...
  // ── FULL READING VIEW ──
  if (readingCompanion) {
    const currentIndex = companions.findIndex(c => c.slug === readingCompanion.slug)
      ...
      <div style={{ minHeight: '100vh', background: '#0D0B12', paddingTop: 64 }}>
        <div style={{ maxWidth: 720, margin: '0 auto', padding: 'clamp(40px, 6vw, 80px) 24px 120px' }}>
          ...
          <motion.button
            ...
            animate={{ opacity: 1 }}
            onClick={() => {
              markAsRead(readingCompanion.slug)
              setReadingCompanion(null)
            }}
            style={{
              display: 'inline-flex',
              alignItems: 'center',
          ...
          >
            {readingCompanion.biography.split('\n\n').map((paragraph, pi) => (
              <p key={pi} style={{
                fontFamily: 'var(--font-sans)',
          ...
          >
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {readingCompanion.virtues.map((virtue, vi) => (
                <div key={vi} style={{
                  padding: '16px 20px',
                  borderRadius: 12,
          ...
          {/* Famous Quotes */}
          {readingCompanion.famousQuotes.length > 0 && (
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            >
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                {readingCompanion.famousQuotes.map((quote, qi) => (
          ...
          >
            {/* Mark read button */}
            <button
              onClick={() => toggleRead(readingCompanion.slug)}
              style={{
                display: 'inline-flex',
                alignItems: 'center',
                gap: 8,
                ...
                fontSize: 13,
                fontWeight: 500,
                color: readSlugs.includes(readingCompanion.slug) ? '#C9A84C' : '#B3B0A6',
                background: readSlugs.includes(readingCompanion.slug)
                  ? 'rgba(201,168,76,0.1)'
                  : 'rgba(255,255,255,0.03)',
                border: readSlugs.includes(readingCompanion.slug)
            ...
            >
              <Check size={16} />
              {readSlugs.includes(readingCompanion.slug) ? 'Perfil lido' : 'Marcar como lido'}
            </button>
        ...
        <div style={{ position: 'relative', zIndex: 1, maxWidth: 800, margin: '0 auto' }}>
          ...
          <BlurFade delay={0.3}>
            <p style={{
              ...
              Escravos que se tornaram lideres. Guerreiros que se tornaram sabios.
              Homens comuns que construiram uma civilizacao. 10 perfis. 10 licoes de vida.
            </p>
          </BlurFade>
        ...
        <div style={{ maxWidth: 720, margin: '0 auto' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {companions.map((companion, i) => {
              const isRead = readSlugs.includes(companion.slug)
              const isOpen = openCompanion === companion.slug
              ...
              return (
                ...
                >
                  <div style={{
                    ...
                    <div
                      onClick={() => setOpenCompanion(isOpen ? null : companion.slug)}
                      style={{
                        ...
                        >
                          <div style={{ padding: '0 24px 28px 76px' }}>
                            ...
                            <p style={{
                              ...
                              {companion.biography.split('\n\n')[0].slice(0, 280)}...
                            </p>

                            {/* Virtues + Quotes count */}
                            <div style={{
                              display: 'flex',
                              alignItems: 'center',
                              gap: 16,
                              ...
                              <div style={{
                                ...
                                <Shield size={13} style={{ color: '#C9A84C', opacity: 0.6 }} />
                                <span style={{
                                  fontFamily: 'var(--font-sans)',
                                  fontSize: 12,
                                  color: '#7A7870',
                                }}>
                                  {companion.virtues.length} virtudes
                                </span>
                              </div>
                              <div style={{
                                display: 'flex',
                                ...
                                <Quote size={13} style={{ color: '#C9A84C', opacity: 0.6 }} />
                                <span style={{
                                  fontFamily: 'var(--font-sans)',
                                  fontSize: 12,
                                  color: '#7A7870',
                                }}>
                                  {companion.famousQuotes.length} citacoes
                                </span>
                              </div>
                            </div>
      ...
      <section style={{
        ...
        <BlurFade delay={0}>
          <p style={{
            ...
            Homens comuns. Fe extraordinaria. Legado eterno.
          </p>
        </BlurFade>
