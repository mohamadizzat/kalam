...
import { financeTopics, type FinanceTopic } from '@/lib/data/islamic-finance'
import { BlurFade } from '@/components/effects/BlurFade'

const STORAGE_KEY = 'kalam-finance-read'
...
function setReadTopics(slugs: string[]) {
  try {
...
export default function FinancasClient() {
  ...
  const [readingTopic, setReadingTopic] = useState<FinanceTopic | null>(null)

  useEffect(() => {
    setReadSlugs(getReadTopics())
  }, [])
  ...
  const total = financeTopics.length
  const progressPercent = total > 0 ? (progress / total) * 100 : 0

  // ── Full reading view ──
  if (readingTopic) {
    const currentIndex = financeTopics.findIndex(t => t.slug === readingTopic.slug)
    const TopicIcon = ICON_MAP[readingTopic.icon] || BookOpen
        ...
        <div style={{ maxWidth: 720, margin: '0 auto', padding: 'clamp(40px, 6vw, 80px) 24px 120px' }}>
          ...
          >
            ...
            <div style={{
              ...
              <div style={{
                ...
                <TopicIcon size={20} style={{ color: '#C9A84C' }} />
              </div>
              <span style={{
                fontFamily: 'var(--font-sans)',
                fontSize: 11,
          ...
          >
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {readingTopic.principles.map((principle, pi) => (
                <div key={pi} style={{
                  display: 'flex',
                  gap: 14,
                  padding: '14px 18px',
          ...
          >
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              {readingTopic.practicalTips.map((tip, ti) => (
                <div key={ti} style={{
                  padding: '16px 20px',
                  borderRadius: 12,
          ...
          >
            {/* Mark read button */}
            <button
              onClick={() => toggleRead(readingTopic.slug)}
              style={{
                display: 'inline-flex',
                alignItems: 'center',
                gap: 8,
                ...
                fontSize: 13,
                fontWeight: 500,
                color: readSlugs.includes(readingTopic.slug) ? '#C9A84C' : '#B3B0A6',
                background: readSlugs.includes(readingTopic.slug)
                  ? 'rgba(201,168,76,0.1)'
                  : 'rgba(255,255,255,0.03)',
                border: readSlugs.includes(readingTopic.slug)
            ...
            >
              <Check size={16} />
              {readSlugs.includes(readingTopic.slug) ? 'Topico estudado' : 'Marcar como estudado'}
            </button>
            ...
            {currentIndex < total - 1 && (
              <button
                onClick={() => {
                  markAsRead(readingTopic.slug)
                  const next = financeTopics[currentIndex + 1]
                  if (next) {
                    setReadingTopic(next)
        ...
        <div style={{ position: 'relative', zIndex: 1, maxWidth: 800, margin: '0 auto' }}>
          ...
          <BlurFade delay={0.3}>
            <p style={{
              ...
              Dinheiro e ferramenta, nao finalidade. Aprenda os principios que
              governam a riqueza no Islam e como aplica-los no Brasil.
            </p>
          </BlurFade>
            ...
            <div style={{ maxWidth: 400, margin: '0 auto' }}>
              <div style={{
                ...
                <span style={{
                  ...
                  {progress}/{total} topicos estudados
                </span>
                <span style={{
                  fontFamily: 'var(--font-sans)',
                  fontSize: 12,
        ...
        <div style={{ maxWidth: 720, margin: '0 auto' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {financeTopics.map((topic, i) => {
              ...
              const TopicIcon = ICON_MAP[topic.icon] || BookOpen
                    ...
                    >
                      ...
                      <div style={{
                        ...
                        {isRead ? (
                          ...
                          <TopicIcon size={16} style={{ color: '#7A7870' }} />
                        )}
                      </div>
                          ...
                          <div style={{ padding: '0 24px 28px 76px' }}>
                            ...
                            <div style={{
                              ...
                              <div style={{
                                ...
                                <BookOpen size={13} style={{ color: '#C9A84C', opacity: 0.6 }} />
                                <span style={{
                                  fontFamily: 'var(--font-sans)',
                                  fontSize: 12,
                                  color: '#7A7870',
                                }}>
                                  {topic.principles.length} principios
                                </span>
                              </div>
                              <div style={{
                                display: 'flex',
                                ...
                                <span style={{
                                  ...
                                  {topic.practicalTips.length} dicas praticas
                                </span>
                              </div>
                            </div>

                            {/* Read full topic button */}
                            <button
                              onClick={(e) => {
                                e.stopPropagation()
      ...
      <section style={{
        ...
        <BlurFade delay={0}>
          <p style={{
            ...
            &ldquo;A riqueza nao diminui pela caridade.&rdquo;
            <span style={{
              display: 'block',
              fontSize: '0.6em',
              color: '#7A7870',
