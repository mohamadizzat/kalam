...
export function DashboardHome() {
  ...
  const personalizedSections = useMemo(() => {
    // During Ramadan, put Praticar first
    if (ramadanToday) {
      const ramadanOrder = ['Praticar', 'Aprender', 'Descobrir', 'Refletir']
      return orderByPersona([...EXPLORE_SECTIONS], ramadanOrder, 'category')
    }
  ...
  }, [persona, ramadanToday])

  const personalizedActions = useMemo(() => {
    const base = [...QUICK_ACTIONS] as Array<{ label: string; href: string; icon: typeof BookOpen }>
    // During Ramadan, add Ramadan quick actions at the start
    if (ramadanToday) {
      const ramadanActions: typeof base = [
        { label: 'Ramadan', href: '/a-jornada/ramadan', icon: Calendar },
      ]
      const ordered = persona
        ? orderByPersona(base, PERSONA_QUICK_PRIORITY[persona] || [], 'href')
        : base
      return [...ramadanActions, ...ordered]
    }
    if (!persona) return base
    const priority = PERSONA_QUICK_PRIORITY[persona] || []
    return orderByPersona(base, priority, 'href')
  }, [persona, ramadanToday])

  return (
    <main style={{ background: T.bg, minHeight: '100vh', position: 'relative' }}>

      {/* Ramadan Atmosphere (stars + gradient + moon) */}
      {ramadanToday && (
